...
import { useCommitImportMutation, usePreviewImportMutation } from "@/lib/api/imports";
import type { ImportPreview, Trade } from "@/lib/api/types";
import {
  AlertCircle,
  ArrowRight,
  CheckCircle,
...
function normalizeSource(source: string): BrokerSource {
  const normalized = source.trim().toLowerCase();
  if (normalized in BADGE_CONFIG) {
    return normalized as BrokerSource;
  }
  return "fidelity";
}

function FormatBadge({ source }: { source: string }) {
  const cfg = BADGE_CONFIG[normalizeSource(source)];
  return (
    <span
      style={{
        display: "inline-flex",
...
function previewTrades(result: ImportPreview): Trade[] {
  return result.rows
    .map((row) => row.normalizedTrade)
...
export default function ImportPage() {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      const preview = await previewImportMutation.mutateAsync({
        file,
  ...
  const handleConfirm = async () => {
    ...
    setStatus("importing");
    try {
      await commitImportMutation.mutateAsync(result.id);
      await reloadTrades();
