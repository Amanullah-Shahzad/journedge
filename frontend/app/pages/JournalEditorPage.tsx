...
export default function JournalEditorPage() {
  ...
  const saveJournalEntryMutation = useSaveJournalEntryMutation();
  ...
  const checkAutoApplyTemplate = useCallback(async (t: Trade) => {
  ...
  }, [activeTradeId, editor, trade]);

  useEffect(() => {
    if (trade && editor && templates.length > 0) {
      void checkAutoApplyTemplate(trade);
    }
  }, [checkAutoApplyTemplate, editor, templates, trade]);
  ...
  const triggerAutosave = useCallback((content: object) => {
    ...
    saveTimer.current = setTimeout(async () => {
      ...
      try {
        ...
        await saveJournalEntryMutation.mutateAsync({
          tradeId: trade.id,
  ...
  }, [saveJournalEntryMutation, trade, updateTradeInMemory]);

  const saveTags = async (newTags: string[]) => {
    if (!trade) return;
    setCurrentTags(newTags);
    updateTradeInMemory(trade.id, { tags: newTags });
    await updateTradeMutation.mutateAsync({ id: trade.id, tags: newTags });
  };

  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file || !editor) return;
    const { url } = await uploadScreenshotMutation.mutateAsync({ file, tradeId: trade?.id || null });
  ...
  const handleSaveTemplate = async (name: string, scope: string) => {
    ...
    await createTemplateMutation.mutateAsync({ name, content, scope });
