"use client";
import { createUpdateEventSource, restartManagedUpdate, useLatestReleaseQuery } from "@/lib/api/settings";
...
export default function SettingsPage() {
  ...
  const latestReleaseQuery = useLatestReleaseQuery();
  ...
  const handleAutoUpdate = () => {
    ...
    const source = createUpdateEventSource();
  ...
  const latestVersion = latestReleaseQuery.data?.tag_name?.replace(/^v/, "") || null;
  const checkingUpdate = latestReleaseQuery.isFetching;
  const updateError = latestReleaseQuery.isError;
          ...
          <Section title="About" icon={Info}>
            <Row label="Version" description="Current installed version">
              <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                ...
                <button
                  onClick={() => void latestReleaseQuery.refetch()}
