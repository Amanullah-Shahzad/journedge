...
export default function AnalyticsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const totalPnl = analyticsSummaryQuery.data.totalPnl;
    const winRate = analyticsSummaryQuery.data.winRate;
    const avgWin = analyticsSummaryQuery.data.avgWin;
    const avgLoss = analyticsSummaryQuery.data.avgLoss;
    const profitFactor = analyticsSummaryQuery.data.profitFactor;

    const initBal = activeAccount?.initialBalance ?? 0;
    const equityCurve = analyticsSummaryQuery.data.equityCurve;
    const daily = analyticsSummaryQuery.data.daily;
    const tradingDays = analyticsSummaryQuery.data.tradingDays;

    const sharpe = analyticsSummaryQuery.data.sharpe;
    const sortino = analyticsSummaryQuery.data.sortino;
    const exp = analyticsSummaryQuery.data.expectancy;

    const { maxDd, maxDdPct, longestDdDays, currentDdDays, ddCurve } =
      drawdownStats(sorted, initBal);
    const calmar = analyticsSummaryQuery.data.calmar;
    ...
    const rolling = analyticsSummaryQuery.data.rolling.length > 0
      ? analyticsSummaryQuery.data.rolling
      : trades.length >= 20 ? rollingWinRate(sorted, 20) : [];
    const dowData = analyticsSummaryQuery.data.dowData.length > 0 ? analyticsSummaryQuery.data.dowData : byDayOfWeek(trades);
    ...
    const localSymbolData = Object.entries(bySymbol)
    ...
    const localTagData = Object.entries(byTag)
    ...
    const localMaeMfeChart = maeFdTrades.slice(-20).map((t, i) => ({
      trade: i + 1,
    ...
    const localOvertradingDays = Object.entries(dayCountMap)
    ...
    disciplineScore -= Math.min(40, localOvertradingDays.length * 8);
    ...
    return {
      ...
      bestTrade,
      worstTrade,
      symbolData: analyticsSummaryQuery.data.symbolData.length > 0 ? analyticsSummaryQuery.data.symbolData : localSymbolData,
      tagData: analyticsSummaryQuery.data.tagData.length > 0 ? analyticsSummaryQuery.data.tagData : localTagData,
      tradingDays,
      hasMaeMfe,
      avgMae,
      avgMfe,
      ...
      exitEff,
      tradeEff,
      maeMfeChart: analyticsSummaryQuery.data.maeMfeChart.length > 0 ? analyticsSummaryQuery.data.maeMfeChart : localMaeMfeChart,
      maeFdCount: maeFdTrades.length,
      overtradingDays: analyticsSummaryQuery.data.overtradingDays.length > 0 ? analyticsSummaryQuery.data.overtradingDays : localOvertradingDays,
      revengeCount: analyticsSummaryQuery.data.revengeCount,
      disciplineScore: analyticsSummaryQuery.data.disciplineScore ?? disciplineScore,
      meanDailyTrades,
      maxDayTrades,
    };
  }, [trades, activeAccount, analyticsSummaryQuery.data]);
              ...
              >
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart data={stats.maeMfeChart} barGap={2}>
                    ...
                    <YAxis tick={axisStyle} axisLine={false} tickLine={false} />
                    <Tooltip contentStyle={tooltipStyle} formatter={(v: any, name: any) => [`$${v}`, name === "mfe" ? "MFE" : "MAE"]} />
